<!-- Start Including Header-->
<?php
include('./mainIncluding/header.php');
?>
<!--End Including Header-->

<!-- Start course page Banner-->
<h5>AMi aci</h5>
<div class="container-fluid bg-dark">
	<div class="row">
		<img src="image/coursebanner.jpg" alt="courses" style="height: 500px; width: 100%;object-fit: cover;box-shadow: 10px;"/>
		
	</div>
	
</div>
<!--End course page Banner-->

<!-- Start Main content-->
 
 <div class="container mt-5">
 	<div class="row">
 		<div class="col-md-4">
 			<img src="./image/course/guitar.jpg" class="card-img-top" alt="Guitar" />
 			
 		</div>
 		<div class="col-md-4">
 			<div class="card-body">
 				<h5 class="card-title">Cousre Name: Learn Guitar</h5>
 				<p class="card-text"> Description: Lorem ipsum dolor sit amet consecteture adipisicing elit. Facilitis, memo.</p>
 				<p class="card-text">Duration: 20 Days</p>
 				<form action="" method="post">
 					<p class="card-text d-inline">Price: <small><del>&#2547 3000</del></small>
  						<span class="font-weight-bolder">&#2547 2000</span></p>
  					<button type="submit" class="btn btn-primary text-white font-weight-bolder float-right" name="buy">Buy Now</button>
 					
 				</form>
 				
 			</div>
 			
 		</div>
 		
 	</div>
 	
 </div>

 <div class="container">
 	<div class="row">
 		<table class="table table-bordered table-hover">
 			<thead>
 				<tr>
 					<th scope="col">Lesson No.</th>
 					<th scope="col">Lesson Name</th>
 				</tr>
 			</thead>
 			<tbody>
 				<tr>
 					<th scope="row">1</th>
 					<td>Introduction</td></tr>
 				</tr>
 			</tbody>
 			
 		</table>
 		
 	</div>
 	
 </div>

<!-- End Main Content-->


<!---Start Including Footer-->
<?php
include('./mainIncluding/footer.php');
?>
<!-- End Including Footer-->